...
void initialize_sequences(){
    ...
    //function for initial CRE and consensus sequence setup
    // currently sequences are being read from a file --> simulations are running over the same set of sequences

    int j;

    
    for(j=0; j<LX; j++) X[j] = rand()%4;
...
void read_initial_sequences(const char *fname){
    ...
    for(int j=0; j<LX; j++) fscanf(f, "%1d ", &X[j]);
...
void print_gene_expression(FILE *out, double t, double g[NE]){

    fprintf(out,"%.10lf ",t);
    for(int j=0; j<NE; j++) fprintf(out,"%.10lf ",g[j]);
...
void print_sequences(FILE *out){
    ...
    for(int i=0; i<NT; i++){
     ...
     //Promoter
    for(int j=0; j<LX; j++) fprintf(out, "%d", X[j]);
        fprintf(out, " ");
